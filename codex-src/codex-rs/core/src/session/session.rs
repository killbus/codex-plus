...
const AUTOMATIC_TURN_ADMISSION_CLAIM: u64 = 1 << 63;
const CLIENT_INPUT_RESERVATION_MASK: u64 = AUTOMATIC_TURN_ADMISSION_CLAIM - 1;

pub(crate) struct AutomaticTurnAdmissionGuard<'a> {
    state: &'a AtomicU64,
}

impl Drop for AutomaticTurnAdmissionGuard<'_> {
    fn drop(&mut self) {
        let previous = self.state.fetch_and(
            CLIENT_INPUT_RESERVATION_MASK,
            std::sync::atomic::Ordering::AcqRel,
        );
        assert!(
            previous & AUTOMATIC_TURN_ADMISSION_CLAIM != 0,
            "automatic turn admission claim was not held"
        );
    }
}
...
pub(crate) struct Session {
    ...
    /// Atomic admission state shared by entered client-input handlers and the
    /// final automatic-turn installation boundary. The high bit is reserved
    /// for an automatic admission claim; the remaining bits count client input.
...
impl Session {
    pub(crate) fn has_client_input_reservations(&self) -> bool {
        self.client_input_reservations
            .load(std::sync::atomic::Ordering::Acquire)
            & CLIENT_INPUT_RESERVATION_MASK
            != 0
    }

    pub(crate) fn reserve_client_input(&self) {
        let result = self.client_input_reservations.fetch_update(
            std::sync::atomic::Ordering::AcqRel,
            std::sync::atomic::Ordering::Acquire,
            |state| {
                let reservations = state & CLIENT_INPUT_RESERVATION_MASK;
                if reservations == CLIENT_INPUT_RESERVATION_MASK {
                    None
                } else {
                    Some(state + 1)
                }
        ...
        assert!(result.is_ok(), "client input reservation overflow");
    }

    pub(crate) fn release_client_input(&self) {
        let result = self.client_input_reservations.fetch_update(
            std::sync::atomic::Ordering::AcqRel,
            std::sync::atomic::Ordering::Acquire,
            |state| {
        ...
        assert!(result.is_ok(), "client input reservation underflow");
    }

    pub(crate) fn try_claim_automatic_turn_admission(
        &self,
    ) -> Option<AutomaticTurnAdmissionGuard<'_>> {
        self.client_input_reservations
            .compare_exchange(
                0,
                AUTOMATIC_TURN_ADMISSION_CLAIM,
                std::sync::atomic::Ordering::AcqRel,
                std::sync::atomic::Ordering::Acquire,
            )
            .ok()
            .map(|_| AutomaticTurnAdmissionGuard {
                state: &self.client_input_reservations,
            })
    }

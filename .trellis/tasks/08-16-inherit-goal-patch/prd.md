...
Goal 补丁迁移到 `codex-plus`，产出可重建的 goal-only 基线；集成树的窄化
transient 行为由 parent 追加，不在本 child 偷换基线含义。
...
- [ ] AC3 concrete `CodexErrorInfo` tests cover usage-limit pause and the inherited
  broad compatibility behavior before the integrated patch is applied.
- [ ] AC4 fixed-toolchain test/build commands are reproducible or explicitly marked
  unavailable on the current host.

## Out of Scope

Transient error narrowing, shadow implementation, and release publication belong to
parent/integrated children.

...
impl ThreadManager {
    ...
    pub async fn spawn_subagent(
        ...
        // Shadow runs receive an extension-supplied sanitized prompt rather than
        // a raw rollout, so credentials and tool payloads cannot cross the
        // boundary. Other subagents retain the existing interrupted fork.
        if !matches!(
            options.thread_source.as_ref(),
            Some(ThreadSource::Feature(source)) if source == "shadow"
        ) {

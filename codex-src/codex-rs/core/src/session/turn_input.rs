...
impl<'a> ClientInputReservation<'a> {
    fn enter_for_request(
        session: &'a Session,
        request: &TurnInputRequest,
        mode: &TurnInputMode,
    ) -> Option<Self> {
        let reserves_client_input = match mode {
            TurnInputMode::StartOrSteer | TurnInputMode::Steer { .. } => true,
        ...
        reserves_client_input.then(|| {
            session.reserve_client_input();
            Self { session }
        })
    }
}
...
impl Drop for ClientInputReservation<'_> {
    fn drop(&mut self) {
        self.session.release_client_input();
    }
}
...
pub(super) async fn handle(
    ...
    let _client_input_reservation =
        ClientInputReservation::enter_for_request(session, &request, &mode);
    #[cfg(test)]
    if _client_input_reservation.is_some() {
        session.wait_at_client_input_entry_test_gate().await;
    }
    match mode {

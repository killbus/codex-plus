...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn guardian_review_retries_disconnected_stream_then_approves() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let approval = serde_json::json!({
        ...
        "user_authorization": "high",
        "outcome": "allow",
        "rationale": "stream retry succeeded",
    })
    .to_string();
    let dropped = || StreamingSseChunk {
        gate: None,
        body: sse(vec![ev_response_created("resp-disconnected")]),
    };
    let (server, _) = start_streaming_sse_server(vec![
        ...
        vec![StreamingSseChunk {
            gate: None,
            body: sse(vec![
                ev_response_created("resp-disconnect-approved"),
                ev_assistant_message("msg-disconnect-approved", &approval),
                ev_completed("resp-disconnect-approved"),
            ]),
        }],
    ])
    .await;
    ...
    let (outcome, metadata) = run_guardian_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        guardian_exec_command_request("shell-disconnect-retry"),
        ApprovalRequestReasons::default(),
        guardian_output_schema(),
        /*external_cancel*/ None,
        /*max_attempts*/ 3,
    ...
    let GuardianReviewOutcome::Completed(assessment) = outcome else {
        panic!("expected guardian assessment after retrying disconnected stream");
    };
    assert_eq!(assessment.outcome, GuardianAssessmentOutcome::Allow);
    assert_eq!(assessment.rationale, "stream retry succeeded");
    assert_eq!(metadata.attempt_count, 2);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn guardian_review_retries_http_5xx_then_approves() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            core_test_support::responses::sse_response(sse(vec![
                ev_response_created("resp-overload-approved"),
                ev_assistant_message("msg-overload-approved", &approval),
                ev_completed("resp-overload-approved"),
            ])),
        ],
    )
    .await;
    ...
    let (outcome, metadata) = run_guardian_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        guardian_exec_command_request("shell-overload-retry"),
        ApprovalRequestReasons::default(),
        guardian_output_schema(),
        /*external_cancel*/ None,
        /*max_attempts*/ 3,
    ...
    let GuardianReviewOutcome::Completed(assessment) = outcome else {
        panic!("expected guardian assessment after retrying HTTP 5xx");
    };
    assert_eq!(assessment.outcome, GuardianAssessmentOutcome::Allow);
    assert_eq!(assessment.rationale, "5xx retry succeeded");
    assert_eq!(metadata.attempt_count, 2);
...
async fn guardian_review_retries_http_429_then_approves() -> anyhow::Result<()> {
    ...
    // The first Guardian attempt consumes its two-request HTTP retry budget;
    // the bounded Guardian loop then starts a second review attempt.
    assert_eq!(metadata.attempt_count, 2);
    assert!(matches!(
        metadata.guardian_session_kind,
        Some(codex_analytics::GuardianReviewSessionKind::TrunkReused)
...
-> anyhow::Result<()> {
    ...
    let overload = || {
        wiremock::ResponseTemplate::new(503).set_body_json(serde_json::json!({
            "error": {
                "code": "server_is_overloaded",
                "message": "persistent reviewer overload",
            }
        }))
    };
    let request_log = mount_response_sequence(
    ...
    let decision = review_approval_request(
        &session,
        &turn,
        "review-exhausted-transient-retry".to_string(),
        guardian_exec_command_request("shell-exhausted-transient-retry"),
        ApprovalRequestReasons::default(),
    )
    .await;

    assert!(matches!(decision, ReviewDecision::Denied { .. }));
    assert_eq!(request_log.requests().len(), 6);
    let mut statuses = Vec::new();
    while let Ok(event) = rx.try_recv() {
        if let EventMsg::GuardianAssessment(event) = event.msg {
            statuses.push(event.status);

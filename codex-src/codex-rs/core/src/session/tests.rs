...
use crate::tasks::AutomaticTurnAdmissionTestGate;
...
#[tokio::test]
async fn inject_if_running_for_turn_rejects_mismatch_and_accepts_match() {
    let (sess, tc, _rx) = make_session_and_context_with_rx().await;
    sess.spawn_task(
        Arc::clone(&tc),
        Vec::new(),
    ...
    let item = user_message("shadow report");
    assert_eq!(
        Err(vec![item.clone()]),
        sess.inject_if_running_for_turn("wrong-turn", vec![item.clone()])
            .await
    );
    assert!(
        sess.inject_if_running_for_turn(&tc.sub_id, vec![item])
            .await
            .is_ok()
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn client_input_registered_before_automatic_claim_rejects_shadow_without_lifecycle() {
    let (sess, _tc, rx) = make_session_and_context_with_rx().await;
    let automatic_gate = AutomaticTurnAdmissionTestGate::default();
    sess.install_automatic_turn_admission_test_gate(automatic_gate.clone());
    let client_gate = super::turn_input::ClientInputEntryTestGate::default();
    sess.install_client_input_entry_test_gate(client_gate.clone());

    let idle_epoch = sess.idle_epoch.load(std::sync::atomic::Ordering::Acquire);
    let display_item = shadow_report_turn_item("shadow-admission-race");
    let idle_session = Arc::clone(&sess);
    let idle_display_item = display_item.clone();
    let idle_start = tokio::spawn(async move {
        idle_session
            .try_start_turn_if_idle_for_epoch_with_origin(
                idle_epoch,
                AutomaticTurnOrigin::Extension("shadow".to_owned()),
                vec![idle_display_item],
                vec![user_message("shadow report")],
            )
            .await
    });
    ...
    timeout(
        ...
        automatic_gate.wait_until_reached(),
    )
    .await
    .expect("automatic turn should reach its final admission boundary");
    ...
    let user_input = tokio::spawn(async move {
        turn_input::handle(
            ...
            TurnInputRequest::user_input(vec![UserInput::Text {
                text: "client input wins admission".to_owned(),
                text_elements: Vec::new(),
            }]),
            TurnInputMode::StartOrSteer,
            "admission-winning-user-turn".to_owned(),
        )
        .await
        .expect("client input should be valid")
    });
    timeout(StdDuration::from_secs(2), client_gate.wait_until_reached())
        .await
        .expect("client input should register before automatic admission");

    automatic_gate.release();
    let idle_error = timeout(StdDuration::from_secs(2), idle_start)
        .await
        .expect("automatic admission should finish after the gate opens")
        .expect("automatic admission must not panic")
        .expect_err("registered client input must win automatic admission");
    assert_eq!(TryStartTurnIfIdleRejectionReason::Busy, idle_error.reason());

    while let Ok(event) = rx.try_recv() {
        match event.msg {
            EventMsg::TurnStarted(_) => {
                panic!("rejected Shadow admission emitted turnStarted")
            }
            EventMsg::ItemStarted(ItemStartedEvent { item, .. })
            | EventMsg::ItemCompleted(ItemCompletedEvent { item, .. })
                if shadow_report_turn_item_matches(&item, &display_item) =>
            {
                panic!("rejected Shadow admission emitted item lifecycle")
            }
            _ => {}
        }
    }
    ...
    client_gate.release();
    let submission = timeout(StdDuration::from_secs(2), user_input)
        .await
        .expect("client input handler should finish after the gate opens")
        .expect("client input handler should not panic");
    assert_eq!(
        submission,
        TurnInputSubmission::Started {
            turn_id: "admission-winning-user-turn".to_owned(),
        }
    );
    ...
    let active_or_completed_turn = match active_turn {
        ...
        None => {
            ...
            assert_eq!(terminal.id, "admission-winning-user-turn");
...
async fn try_start_turn_if_idle_rejects_in_flight_client_user_input() {
    let (sess, _tc, rx) = make_session_and_context_with_rx().await;
    let gate = super::turn_input::ClientInputEntryTestGate::default();
    ...
    let user_input = tokio::spawn(async move {
        ...
        assert_eq!(
            submission,
            TurnInputSubmission::Started {
                turn_id: "in-flight-user-turn".to_owned(),
            }
        );
    });
...
async fn steer_input_requires_active_turn() {
    ...
    let submission = submit_steer_only(&sess, input, "missing-turn").await;
    assert_eq!(
        submission,
...
async fn steer_input_enforces_expected_turn_id() {
    ...
    let submission = submit_steer_only(&sess, steer_input, "different-turn-id").await;

    match submission {
        TurnInputSubmission::NotSubmitted {
            reason: NotSubmittedReason::ExpectedTurnMismatch { expected, actual },
        } => {
            assert_eq!(
                (expected, actual),

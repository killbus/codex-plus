...
在 integrated `codex-plus` 基线上构建并发布六个原生平台 CLI archive：Windows
x64/ARM64、macOS x64/ARM64 和 Linux musl x64/ARM64。每份归档提供完整许可证/声明、
非官方商标边界、来源与补丁 provenance、SHA-256，并以精确白名单拒绝任何未声明文件。

## Requirements

- R1 workflow 固定 upstream commit、patch hashes、Cargo.lock、Rust `1.95.0`
  和每个目标 triple；先跑 Goal/shadow 回归，再检查 app-server 并构建 `codex-cli`。
- R2 archive 只允许当前平台的 `codex`/`codex.exe`、LICENSE、NOTICE、
  TRADEMARKS.md、SHA256SUMS 和 BUILD-INFO.txt。
...
- [ ] AC1 workflow 在六个原生 runner 上成功测试、构建并生成 platform archive。
...
非 CLI 发行物、marketplace 上传和字节级 deterministic build。

...
async fn dropped_stream_goal_turn_error_starts_next_automatic_turn_with_shadow() -> Result<()> {
    ...
    let (server, _) = start_streaming_sse_server(vec![
        stream(create_goal),
        stream(dropped_after_handshake("dropped-first-attempt")),
        stream(dropped_after_handshake("dropped-first-retry")),
        stream(dropped_after_handshake("dropped-second-attempt")),
        stream(dropped_after_handshake("dropped-second-retry")),
    ...
    std::fs::write(
        &config_path,
        config.replace("stream_max_retries = 0", "stream_max_retries = 1"),
    )?;
    let mut mcp = TestAppServer::builder()
        .with_codex_home(codex_home.path())
        .without_managed_config()
    ...
    let failed_again: TurnCompletedNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_notification("turn/completed"),
    )
    .await??;
    assert_eq!(failed_again.turn.id, continued.turn.id);
    assert_eq!(failed_again.turn.status, TurnStatus::Failed);
    assert!(failed_again.turn.error.is_some());

    let continued_again_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_matching_notification(
            "second automatic turn/started",
            |notification| {
                notification.method == "turn/started"
                    && notification
                        .params
    ...
    .await??;
    let continued_again: TurnStartedNotification = serde_json::from_value(
        continued_again_notification
            .params
            .expect("second turn/started params"),
    )?;
    assert_eq!(continued_again.thread_id, thread.id);
    assert_ne!(continued_again.turn.id, turn.id);
    assert_ne!(continued_again.turn.id, continued.turn.id);
    ...
    .await??;
    assert_eq!(completed.turn.id, continued_again.turn.id);
...
async fn run_transient_http_goal_turn_error(
    ...
    let continued_again_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_matching_notification(
            "second automatic turn/started after transient HTTP error",
            |notification| {
                notification.method == "turn/started"
                    && notification
                        .params

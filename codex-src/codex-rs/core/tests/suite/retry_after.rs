...
/// Ordinary HTTP 429 responses use the same bounded request backoff as transient 5xx errors.
#[tokio::test(flavor = "current_thread")]
async fn responses_http_429_retries_with_local_backoff() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    submit_user_input(&test, "retry the temporary rate limit").await?;
    let retry = telemetry.next_retry().await;
    assert!((FIRST_RETRY_MIN_DELAY..FIRST_RETRY_MAX_DELAY).contains(&retry.delay));
    assert_eq!(
        retry,
...
/// Ordinary HTTP 429 responses exhaust the bounded request budget before one terminal error.
#[tokio::test(flavor = "current_thread")]
async fn responses_http_429_exhausts_request_retries() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    submit_user_input(&test, "exhaust the temporary rate limit").await?;
    let mut error_events = 0;
    let mut stream_error_events = 0;
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
    ...
    .await
    .expect("HTTP 429 retries should finish the turn within 10 seconds");
...
/// Retrying 429 does not collapse the final usage-limit response into a generic retry error.
#[tokio::test(flavor = "current_thread")]
async fn responses_http_usage_limit_remains_distinct_after_request_retries() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    submit_user_input(&test, "preserve the usage-limit classification").await?;
    let mut error_events = 0;
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            match wait_for_event(&test.codex, |_| true).await {
    ...
    .await
    .expect("usage-limit retries should finish the turn within 10 seconds");

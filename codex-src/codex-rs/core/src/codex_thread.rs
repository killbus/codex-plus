...
pub enum TryStartTurnIfIdleRejectionReason {
    ...
    Busy,
    /// The idle edge that authorized this work is no longer current.
    StaleIdleEpoch,
}
...
impl CodexThread {
    ...
    /// Injects items only while the expected turn is still active.
    pub async fn inject_if_running_for_turn(
        ...
        self.session
            .inject_if_running_for_turn(expected_turn_id, items)
            .await
    }
    ...
    /// Starts automatic idle work only while the observed idle epoch is current.
    pub async fn try_start_turn_if_idle_for_epoch(
        &self,

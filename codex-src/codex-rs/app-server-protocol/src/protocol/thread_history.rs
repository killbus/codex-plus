...
mod tests {
    ...
    use codex_extension_items::shadow::ShadowReportItem as CoreShadowReportItem;
    ...
    #[test]
    fn rebuilds_shadow_report_from_persisted_completion() {
        let turn_id = "turn-1";
        let thread_id = ThreadId::new();
        let shadow_report = CoreShadowReportItem {
            id: "shadow-report-1".to_string(),
            shadow_id: "reviewer".to_string(),
            shadow_name: "Reviewer".to_string(),
            content: "Check the persistence boundary.".to_string(),
        };
        let events = vec![
            EventMsg::TurnStarted(TurnStartedEvent {
                turn_id: turn_id.to_string(),
            ...
            EventMsg::ItemCompleted(ItemCompletedEvent {
                thread_id,
                turn_id: turn_id.to_string(),
                item: CoreTurnItem::Extension(CoreExtensionItem::ShadowReport(
                    shadow_report.clone(),
                )),
                completed_at_ms: 1_000,
            }),
        ...
        assert_eq!(
            turns[0].items,
            vec![ThreadItem::ShadowReport(shadow_report)]
        );
    }
